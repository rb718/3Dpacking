...
class Item:
    def __init__(self, **context):
        self.name = context['description']
        self.length = context['length']
        self.width = context['width']
        self.height = context['depth']
        self.weight = context['weight']
        self.packaged_in_own_box = context['packaged_in_own_box']
        self.rotation_type = 0 # initial rotation type: (x, y, z) --> (l, w, h)
        self.position = START_POSITION # initial position: (0, 0, 0)
        self.number_of_decimals = DEFAULT_NUMBER_OF_DECIMALS
        self.context = context #all the args from the input
        self.packaged_already = context['packaged_already'] #update for already packaged items
